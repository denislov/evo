//! TypeScript / TSX grammar / query 配置。
//!
//! `.scm` 查询文本与 namespaces 直接移植 Grok（数据 / 契约层）；扩展名
//! 覆盖 `ts` / `tsx` / `mts` / `cts`（Evo 注册表基线）。

// Adapted from xai-codebase-graph, SOURCE_REV d6937fe255dce4133c3d000a50f9cb94de12f06f
// (languages/ts.rs); query text and namespaces copied verbatim, language ids
// normalized to Evo's lowercase ids, mts/cts extensions retained from the
// Evo skeleton registry.
use crate::languages::LanguageConfig;

pub fn ts_lang() -> LanguageConfig {
    LanguageConfig::with_grammar(
        vec!["typescript".to_owned(), "ts".to_owned()],
        vec![
            "ts".to_owned(),
            "tsx".to_owned(),
            "mts".to_owned(),
...
/// TypeScript definitions query（移植 Grok；覆盖函数 / 类 / 接口 / 类型 /
/// 枚举 / 变量 / 解构定义 + 成员 / import / export / JSX / 调用引用 +
/// alias 对）。
