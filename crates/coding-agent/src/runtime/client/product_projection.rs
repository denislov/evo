...
mod tests {
    ...
    use crate::events::{
        ...
        CodingAgentProductEventUsage, CodingAgentSessionProductEvent, CodingAgentTeamProductEvent,
    ...
    use crate::runtime::client::projection::{
        CodingAgentDelegationSnapshot, CodingAgentDraft, CodingAgentDraftId, CodingAgentDraftKind,
        CodingAgentFileChangeSnapshot, CodingAgentOperationSnapshot,
    };
    use crate::runtime::facade::{
        CapabilityStatus, CodingAgentCapabilities, CodingAgentContextSnapshot,
        CodingAgentImageContent, CodingAgentOperationStatus, CodingAgentSessionView,
    ...
    fn inventory_class(event: &CodingAgentProductEventKind) -> InventoryClass {
        match event {
    ...
    fn capabilities() -> CodingAgentCapabilities {
        CodingAgentCapabilities {
            ...
            switch_session: CapabilityStatus::Unsupported {
                reason: "session switching is unavailable".into(),
            },
    ...
    fn snapshot(last_sequence: u64) -> CodingAgentSnapshot {
        CodingAgentSnapshot {
    ...
    fn event(
        ...
        let operation_id = match &event {
            CodingAgentProductEventKind::Profile(_)
            | CodingAgentProductEventKind::Capability(_)
            | CodingAgentProductEventKind::Runtime(CodingAgentRuntimeProductEvent::ShutDown) => {
                None
            }
            _ => Some("operation-1".into()),
        };
        CodingAgentProductEvent::from_draft_for_tests(
            ...
            ProductEventDraft {
                ...
                terminal_status: terminal.map(|value| value.status),
    ...
    fn transcript(session_id: &str) -> CodingAgentTranscriptSnapshot {
        CodingAgentTranscriptSnapshot {
            ...
            items: vec![
                ...
                CodingAgentSessionTranscriptItem::Delegation {
                    ...
                    target_id: ProfileId::from("reviewer"),
                    task: "review".into(),
    ...
    fn pending_recovery(recovery_id: &str) -> CodingAgentRecoveryPending {
    ...
    #[test]
    fn invalid_event_requests_resync_without_mutating_product_facts_or_cursor() {
        let mut projection = CodingAgentClientProjection::new(snapshot(0)).unwrap();
        ...
        assert!(matches!(
            ...
            CodingAgentClientProjectionApply::NeedsResync(CodingAgentClientProjectionIssue {
                code: "product_event_cursor_gap",
                ..
            })
        ));
    ...
    #[test]
    fn duplicate_is_idempotent_and_does_not_change_lifecycle() {
        let mut projection = CodingAgentClientProjection::new(snapshot(1)).unwrap();
    ...
    #[test]
    fn ordered_events_fold_shared_operation_message_usage_and_profile_state() {
        let mut projection = CodingAgentClientProjection::new(snapshot(0)).unwrap();
        let started = event(
            1,
            CodingAgentProductEventKind::Workflow(CodingAgentWorkflowProductEvent::PromptStarted {
        ...
        let terminal = CodingAgentProductEventTerminalOperation {
        ...
        for next in [&started, &message, &completed, &profile] {
            assert!(matches!(
        ...
        assert_eq!(projection.snapshot().cursor.last_event_sequence, 4);
        assert_eq!(
            projection.snapshot().context.operations[0].status,
            CodingAgentOperationStatus::Completed
        );
        assert_eq!(projection.messages()[0].text, "done");
        assert_eq!(projection.snapshot().context.usage.input, 10);
        assert_eq!(
            projection
                .snapshot()
                .session
    ...
    #[test]
    fn replacement_is_validated_before_existing_projection_is_swapped() {
        let mut projection = CodingAgentClientProjection::new(snapshot(0)).unwrap();
        let before = projection.clone();
        let mut invalid = snapshot(9);
        invalid.cursor.stream_id.clear();

        assert!(matches!(
            projection.replace_snapshot(invalid),
            Err(CodingAgentClientProjectionIssue {
                code: "snapshot_stream_missing",
                ..
            })
        ));
        assert_eq!(projection, before);
    ...
    #[test]
    fn product_event_inventory_is_exhaustive_and_cursor_only_facts_are_explicit() {
        let cursor_only =
        ...
        assert_eq!(inventory_class(&cursor_only), InventoryClass::CursorOnly);
        assert_eq!(inventory_class(&retained), InventoryClass::RetainedState);
        assert_ne!(
            InventoryClass::SnapshotReplacement,
            InventoryClass::ExplicitResync
        );
    }

    #[test]
    fn bootstrap_atomically_installs_transcript_and_authoritative_recoveries() {
        let projection = CodingAgentClientProjection::from_bootstrap(CodingAgentClientBootstrap {
            snapshot: snapshot(7),
            transcript: transcript("session-1"),
            pending_recoveries: vec![pending_recovery("recovery-1")],
        })
        .unwrap();

        let transcript = projection.transcript().unwrap();
        assert_eq!(transcript.session_id(), "session-1");
        assert_eq!(transcript.active_leaf_id(), Some("leaf-1"));
        assert_eq!(transcript.items().len(), 3);
        assert!(!transcript.truncated());
        assert_eq!(projection.recoveries().len(), 1);
    ...
    #[test]
    fn metadata_snapshot_replacement_preserves_transcript_and_recovery_facts() {
        let mut projection =
            CodingAgentClientProjection::from_bootstrap(CodingAgentClientBootstrap {
                snapshot: snapshot(7),
                transcript: transcript("session-1"),
                pending_recoveries: vec![pending_recovery("recovery-1")],
            })
            .unwrap();
        let transcript_before = projection.transcript().cloned();
        let recoveries_before = projection.recoveries().clone();
        let mut metadata = snapshot(8);
        metadata.session.default_agent_profile_id = ProfileId::from("reviewer");

        projection.replace_snapshot(metadata).unwrap();

        assert_eq!(projection.transcript(), transcript_before.as_ref());
        assert_eq!(projection.recoveries(), &recoveries_before);
        assert_eq!(
            projection
                .snapshot()
                .session
    ...
    #[test]
    fn invalid_bootstrap_replacement_leaves_all_existing_state_unchanged() {
        let mut projection =
            CodingAgentClientProjection::from_bootstrap(CodingAgentClientBootstrap {
                snapshot: snapshot(7),
                transcript: transcript("session-1"),
            ...
            .unwrap();
        let before = projection.clone();
        let mut different_session = snapshot(0);
        different_session.session.session_id = "session-2".into();

        assert!(matches!(
            projection.replace_bootstrap(CodingAgentClientBootstrap {
                snapshot: different_session,
                transcript: transcript("session-1"),
    ...
    #[test]
    fn narrow_recovery_replacement_rejects_duplicates_without_mutation() {
        let mut projection =
            CodingAgentClientProjection::from_bootstrap(CodingAgentClientBootstrap {
                snapshot: snapshot(0),
                transcript: transcript("session-1"),
        ...
        assert!(matches!(
            projection.replace_pending_recoveries(vec![
                pending_recovery("duplicate"),
                pending_recovery("duplicate"),
            ]),
            Err(CodingAgentClientProjectionIssue {
                code: "recovery_identity_duplicate",
                ..
            })
        ));
        assert_eq!(projection, before);
    ...
    #[test]
    fn live_event_bounds_matrix_caps_every_retained_collection_and_payload() {
        let mut projection = CodingAgentClientProjection::new(snapshot(0)).unwrap();
        let mut sequence = 0;

        for index in 0..MAX_MESSAGES + 2 {
            sequence += 1;
            let final_text = if index == MAX_MESSAGES + 1 {
                "m".repeat(MAX_MESSAGE_BYTES + 17)
            } else {
            ...
            let next = event(
                ...
                CodingAgentProductEventKind::Message(CodingAgentMessageProductEvent::Completed {
                    operation_id: "operation-1".into(),
                    turn_id: format!("message-turn-{index}"),
        ...
        assert_eq!(projection.messages().len(), MAX_MESSAGES);
        assert_eq!(
            projection.messages().back().unwrap().text.len(),
            MAX_MESSAGE_BYTES
        );
        assert!(projection.messages().back().unwrap().truncated);

        for index in 0..MAX_TOOLS + 2 {
            sequence += 1;
            let summary = if index == MAX_TOOLS + 1 {
                "t".repeat(MAX_TOOL_BYTES + 17)
            } else {
                format!("tool-{index}")
            };
            let next = event(
        ...
        assert_eq!(projection.tools().len(), MAX_TOOLS);
        assert_eq!(
            projection.tools().back().unwrap().detail.len(),
            MAX_TOOL_BYTES
        );
        assert!(projection.tools().back().unwrap().truncated);

        for index in 0..MAX_DIAGNOSTICS + 2 {
            sequence += 1;
            let summary = if index == MAX_DIAGNOSTICS + 1 {
                "d".repeat(MAX_DIAGNOSTIC_BYTES + 17)
            } else {
                ...
                CodingAgentProductEventKind::Diagnostic(
                    CodingAgentDiagnosticProductEvent::Diagnostic {
                        diagnostic: CodingAgentPublicDiagnostic {
                            ...
                            summary,
                            origin: CodingAgentPublicDiagnosticOrigin::Runtime,
        ...
        assert_eq!(projection.diagnostics().len(), MAX_DIAGNOSTICS);
        assert_eq!(
            projection.diagnostics().back().unwrap().summary.len(),
            MAX_DIAGNOSTIC_BYTES
        );
        assert!(projection.diagnostics().back().unwrap().truncated);

        for index in 0..MAX_RECOVERIES + 2 {
            sequence += 1;
            let reason = if index == MAX_RECOVERIES + 1 {
                "r".repeat(MAX_DIAGNOSTIC_BYTES + 17)
            } else {
            ...
            let next = event(
                ...
                CodingAgentProductEventKind::Workflow(
                    CodingAgentWorkflowProductEvent::OperationRecoveryPending {
                        operation_id: "operation-1".into(),
                        recovery_id: format!("recovery-{index}"),
                        reason,
                        record_version: 1,
                        descriptor_revision: 1,
                        capability_generation: Some(0),
        ...
        assert_eq!(projection.recoveries().len(), MAX_RECOVERIES);
        assert_eq!(
            projection.recoveries().front().unwrap().reason.len(),
            MAX_DIAGNOSTIC_BYTES
        );
        assert_eq!(
            projection.recoveries().front().unwrap().recovery_id,
            format!("recovery-{}", MAX_RECOVERIES + 1)
        );
    }

    #[test]
    fn invalid_event_matrix_preserves_all_retained_product_facts() {
        let fixture: Vec<CodingAgentProductEvent> = serde_json::from_str(include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/fixtures/client_projection/cross-adapter-events.json"
        )))
        .unwrap();
        let mut baseline = CodingAgentClientProjection::new(snapshot(0)).unwrap();
        for next in fixture {
            assert!(matches!(
                baseline.apply(&next),
                CodingAgentClientProjectionApply::Applied(_)
        ...
        let mut wrong_stream = serde_json::to_value(&valid_next).unwrap();
        wrong_stream["stream_id"] = serde_json::json!("wrong-stream");
        invalid_values.push(wrong_stream);
        let mut wrong_session = serde_json::to_value(&valid_next).unwrap();
        wrong_session["session_id"] = serde_json::json!("wrong-session");
        invalid_values.push(wrong_session);
        let mut gap = serde_json::to_value(&valid_next).unwrap();
        gap["sequence"] = serde_json::json!(15);
        invalid_values.push(gap);
        let mut wrong_generation = serde_json::to_value(&valid_next).unwrap();
        wrong_generation["capability_generation"] = serde_json::json!(9);
        invalid_values.push(wrong_generation);

        for value in invalid_values {
            let invalid: CodingAgentProductEvent = serde_json::from_value(value).unwrap();
            let mut candidate = baseline.clone();
            let before = candidate.clone();

            assert!(matches!(
                candidate.apply(&invalid),
                CodingAgentClientProjectionApply::NeedsResync(_)
            ));
            assert_eq!(candidate.snapshot(), before.snapshot());
            assert_eq!(candidate.transcript(), before.transcript());
            assert_eq!(candidate.messages(), before.messages());
            assert_eq!(candidate.tools(), before.tools());
            assert_eq!(candidate.diagnostics(), before.diagnostics());
            assert_eq!(candidate.recoveries(), before.recoveries());
            assert_eq!(
                candidate.lifecycle(),
                CodingAgentClientProjectionLifecycle::NeedsResync
            );
            assert!(candidate.resync_issue().is_some());
        }
    }

    #[test]
    fn narrow_replacement_atomicity_matrix_rejects_without_partial_mutation() {
        let mut projection =
            CodingAgentClientProjection::from_bootstrap(CodingAgentClientBootstrap {
                snapshot: snapshot(7),
                transcript: transcript("session-1"),
        ...
        let before_transcript = projection.clone();
        assert!(matches!(
            projection.replace_transcript(transcript("wrong-session")),
            Err(CodingAgentClientProjectionIssue {
                code: "transcript_session_mismatch",
                ..
            })
        ));
        assert_eq!(projection, before_transcript);

        let before_recoveries = projection.clone();
        let mut future = pending_recovery("future-generation");
        future.capability_generation = Some(1);
        assert!(matches!(
            projection.replace_pending_recoveries(vec![future]),
            Err(CodingAgentClientProjectionIssue {
                code: "recovery_capability_generation_invalid",
                ..
            })
        ));
        assert_eq!(projection, before_recoveries);

        let before_snapshot = projection.clone();
        let mut regressed = snapshot(6);
        regressed.session.default_agent_profile_id = ProfileId::from("must-not-leak");
        assert!(matches!(
            projection.replace_snapshot(regressed),
            Err(CodingAgentClientProjectionIssue {
                code: "snapshot_cursor_regression",
                ..
            })
        ));
        assert_eq!(projection, before_snapshot);
    }

    #[test]
    fn snapshot_and_bootstrap_bounds_matrix_caps_top_level_and_transcript_state() {
        let mut bounded_snapshot = snapshot(0);
        bounded_snapshot.context.operations = (0..MAX_CONTEXT_OPERATIONS + 2)
            .map(|index| CodingAgentOperationSnapshot {
                operation_id: format!("operation-{index}"),
                kind: if index == 0 {
                    "k".repeat(MAX_ID_BYTES + 9)
                } else {
                    "prompt".into()
                },
                ...
                started_sequence: index as u64,
                updated_sequence: index as u64,
                diagnostics: if index == 0 {
                    vec!["d".repeat(MAX_DIAGNOSTIC_BYTES + 9); 6]
                } else {
                    Vec::new()
                },
                failure: (index == 0).then(|| "f".repeat(MAX_DIAGNOSTIC_BYTES + 9)),
            })
            .collect();
        bounded_snapshot.context.changes = (0..MAX_CONTEXT_CHANGES + 2)
            .map(|index| CodingAgentFileChangeSnapshot {
                path: if index == 0 {
                    "p".repeat(MAX_TOOL_BYTES + 9)
                } else {
                    format!("file-{index}")
                },
                mutation_kind: if index == 0 {
                    "m".repeat(MAX_ID_BYTES + 9)
                } else {
                    "write".into()
                },
                operation_id: format!("operation-{index}"),
                ...
                added_lines: None,
                removed_lines: None,
                diff: Some(if index == 0 {
                    "d".repeat(MAX_DIFF_BYTES + 9)
                } else {
                    "diff".into()
                }),
            })
            .collect();
        bounded_snapshot.context.delegations = (0..MAX_CONTEXT_DELEGATIONS + 2)
            .map(|index| CodingAgentDelegationSnapshot {
                tool_call_id: format!("delegation-{index}"),
                child_operation_id: Some(format!("child-{index}")),
                target_kind: "agent".into(),
                target_id: format!("target-{index}"),
                task: if index == 0 {
                    "t".repeat(MAX_MESSAGE_BYTES + 9)
                } else {
                    "task".into()
                },
                status: if index == 0 {
                    "s".repeat(MAX_ID_BYTES + 9)
                } else {
                    "completed".into()
                },
                updated_sequence: index as u64,
                summary: Some(if index == 0 {
                    "s".repeat(MAX_MESSAGE_BYTES + 9)
                } else {
                    "done".into()
                }),
                failure: (index == 0).then(|| "f".repeat(MAX_DIAGNOSTIC_BYTES + 9)),
            ...
            .collect();
        bounded_snapshot.drafts = (0..MAX_PENDING_AUTHORIZATIONS + 2)
            .map(|index| CodingAgentDraft {
                id: CodingAgentDraftId(format!("draft-{index}")),
                kind: CodingAgentDraftKind::Prompt,
                text: if index == 0 {
                    "d".repeat(MAX_MESSAGE_BYTES + 9)
                } else {
                    "draft".into()
                },
            })
            .collect();
        bounded_snapshot.pending_authorizations = (0..MAX_PENDING_AUTHORIZATIONS + 2)
            .map(|index| ToolAuthorizationRequest {
                authorization_id: format!("authorization-{index}"),
                operation_id: "operation-1".into(),
                turn_id: "turn-1".into(),
                tool_call_id: format!("authorization-tool-{index}"),
                tool_name: "write".into(),
                risk: ToolAuthorizationRisk::FilesystemMutation,
                scope: ToolAuthorizationScope::Path {
                    path: if index == 0 {
                        "p".repeat(MAX_AUTHORIZATION_BYTES + 9)
                    } else {
                        "file".into()
                    },
                },
                preview: ToolAuthorizationPreview {
                    summary: if index == 0 {
                        "s".repeat(MAX_AUTHORIZATION_BYTES + 9)
                    } else {
                        "summary".into()
                    },
                    ...
                    command: None,
                    cwd: None,
                    content_preview: Some(if index == 0 {
                        "c".repeat(MAX_AUTHORIZATION_BYTES + 9)
                    } else {
                        "content".into()
                    }),
                },
                capability_generation: 0,
                requested_at: format!("requested-{index}"),
            })
            .collect();

        let bounded = CodingAgentClientProjection::new(bounded_snapshot).unwrap();
        assert_eq!(
            bounded.snapshot().context.operations.len(),
            MAX_CONTEXT_OPERATIONS
        );
        assert_eq!(
            bounded.snapshot().context.operations[0].diagnostics.len(),
            4
        );
        assert!(
            bounded.snapshot().context.operations[0]
                .diagnostics
                .iter()
                .all(|value| value.len() == MAX_DIAGNOSTIC_BYTES)
        );
        assert_eq!(
            bounded.snapshot().context.changes.len(),
            MAX_CONTEXT_CHANGES
        );
        assert_eq!(
            bounded.snapshot().context.changes[0]
                .diff
                .as_ref()
                .unwrap()
                .len(),
        ...
        assert_eq!(
            bounded.snapshot().context.delegations.len(),
            MAX_CONTEXT_DELEGATIONS
        );
        assert_eq!(
            bounded.snapshot().context.delegations[0].task.len(),
            MAX_MESSAGE_BYTES
        );
        assert_eq!(bounded.snapshot().drafts.len(), MAX_PENDING_AUTHORIZATIONS);
        assert_eq!(bounded.snapshot().drafts[0].text.len(), MAX_MESSAGE_BYTES);
        assert_eq!(
            bounded.snapshot().pending_authorizations.len(),
            MAX_PENDING_AUTHORIZATIONS
        );
        assert_eq!(
            bounded.snapshot().pending_authorizations[0]
                .preview
                .summary
                .len(),
        ...
        let mut transcript_items = (0..MAX_TRANSCRIPT_ITEMS + 2)
        ...
        transcript_items.push(CodingAgentSessionTranscriptItem::Assistant {
            id: "assistant".into(),
            text: "a".repeat(MAX_MESSAGE_BYTES + 9),
            thinking: "t".repeat(MAX_THINKING_BYTES + 9),
            images: (0..MAX_TRANSCRIPT_IMAGES_PER_ITEM + 2)
                .map(|_| CodingAgentImageContent {
        ...
        transcript_items.push(CodingAgentSessionTranscriptItem::Assistant {
            ...
            thinking: String::new(),
            images: vec![
                CodingAgentImageContent {
                    mime_type: "image/png".into(),
                    data: "i".repeat(MAX_TRANSCRIPT_IMAGE_BYTES + 1),
                },
                CodingAgentImageContent {
                    mime_type: "image/png".into(),
                    data: "retained".into(),
        ...
        assert!(transcript.omitted_items() >= 4);
        assert!(transcript.truncated());
        assert!(transcript.retained_bytes() <= MAX_TRANSCRIPT_BYTES);
        assert_eq!(projection.recoveries().len(), MAX_RECOVERIES);
        let assistant_items = transcript
            .items()
            .iter()
            .filter_map(|item| {
                let CodingAgentSessionTranscriptItem::Assistant {
                    text,
                    thinking,
                    images,
                    ..
                ...
                Some((text, thinking, images))
            })
            .collect::<Vec<_>>();
        assert_eq!(assistant_items.len(), 2);
        assert_eq!(assistant_items[0].0.len(), MAX_MESSAGE_BYTES);
        assert_eq!(assistant_items[0].1.len(), MAX_THINKING_BYTES);
        assert_eq!(assistant_items[0].2.len(), MAX_TRANSCRIPT_IMAGES_PER_ITEM);
        assert_eq!(assistant_items[1].2.len(), 1);
        assert_eq!(assistant_items[1].2[0].data, "retained");
    }
}
