...
mod tests {
    ...
    use crate::tools::filesystem::read::{ReadOperations, RealReadOperations};
    use crate::tools::filesystem::write::{RealWriteOperations, WriteOperations};
    ...
    fn prompt_snapshot_grants_model_tools_session_and_plugin_sets() {
        ...
        let snapshot = service.snapshot(input(OperationKind::Prompt)).unwrap();
    ...
    fn shell_snapshot_freezes_execution_configuration() {
        ...
        let snapshot = CapabilitySnapshotService::new().snapshot(input).unwrap();
        let shell = snapshot.shell.expect("bash capability should be granted");

        assert_eq!(shell.cwd, test_cwd());
    ...
    fn prompt_snapshot_grants_filesystem_for_every_filesystem_tool() {
        ...
        for tool in ["read", "write", "edit", "grep", "find", "ls"] {
            ...
            let snapshot = service.snapshot(input).unwrap();
    ...
    fn compact_snapshot_grants_model_and_session_write() {
        let snapshot = CapabilitySnapshotService::new()
            .snapshot(input(OperationKind::Compact))
            .unwrap();
    ...
    fn runtime_write_install_generation_advances_future_snapshots() {
        ...
        let first = service.snapshot(input(OperationKind::Prompt)).unwrap();

        let installed = service
            .install_next_generation(CapabilityRevocationPolicy::FutureOnly)
            .unwrap();
        let second = service.snapshot(input(OperationKind::Prompt)).unwrap();
    ...
    fn capability_generation_exhaustion_fails_closed() {
        let error = CapabilityGeneration::new(u64::MAX).next().unwrap_err();
        assert!(matches!(
            error,
            CodingSessionError::UnsupportedCapability { capability }
                if capability.contains("generation is exhausted")
        ));
    }
    ...
    fn export_snapshot_grants_read_without_write_or_model() {
        ...
        let snapshot = service.snapshot(input(OperationKind::Export)).unwrap();
    ...
    fn explicit_session_access_is_not_inferred_from_operation_kind() {
        ...
        let snapshot = service.snapshot(request).unwrap();
    ...
    #[test]
    fn filesystem_target_rejects_absolute_paths_outside_root() {
        let root = tempfile::tempdir().unwrap();
        let outside = tempfile::tempdir().unwrap();
        let capability = FilesystemCapability::new(root.path().to_path_buf()).unwrap();

        let error = capability
            .target(outside.path().join("outside.txt"))
            .unwrap_err();
    ...
    #[test]
    fn filesystem_target_rejects_dotdot_escape() {
        let root = tempfile::tempdir().unwrap();
        let capability = FilesystemCapability::new(root.path().to_path_buf()).unwrap();
        let error = capability.target("../outside.txt").unwrap_err();
        assert_eq!(error.code(), "unsupported_capability");
    }

    #[test]
    fn filesystem_target_allows_relative_and_absolute_paths_within_root() {
        let root = tempfile::tempdir().unwrap();
        let capability = FilesystemCapability::new(root.path().to_path_buf()).unwrap();
        let relative = capability.target("src/main.rs").unwrap();
        let absolute = capability.target(root.path().join("src/main.rs")).unwrap();
        assert_eq!(relative.display_path(), root.path().join("src/main.rs"));
        assert_eq!(relative.relative_path(), Path::new("src/main.rs"));
        assert_eq!(absolute.relative_path(), Path::new("src/main.rs"));
    }

    #[tokio::test]
    async fn authorization_bound_read_keeps_the_opened_file_across_path_swap() {
        let root = tempfile::tempdir().unwrap();
        std::fs::write(root.path().join("target.txt"), "authorized").unwrap();
        let capability = FilesystemCapability::new(root.path().to_path_buf()).unwrap();
        capability
            .bind_tool_target("op", "call", "read", "target.txt")
            .await
            .unwrap();

        std::fs::rename(
            root.path().join("target.txt"),
            root.path().join("authorized.txt"),
        )
        .unwrap();
        std::fs::write(root.path().join("target.txt"), "replacement").unwrap();

        let target = capability
            .take_bound_tool_target("op", "call", "read", "target.txt")
            .unwrap();
        let bytes = RealReadOperations.read_file(&target).await.unwrap();
        assert_eq!(bytes, b"authorized");
    }

    #[tokio::test]
    async fn authorization_bound_write_mutates_opened_file_not_path_replacement() {
        let root = tempfile::tempdir().unwrap();
        std::fs::write(root.path().join("target.txt"), "authorized").unwrap();
        let capability = FilesystemCapability::new(root.path().to_path_buf()).unwrap();
        capability
            .bind_tool_target("op", "call", "write", "target.txt")
            .await
            .unwrap();
        ...
        let target = capability
            .take_bound_tool_target("op", "call", "write", "target.txt")
            .unwrap();
        RealWriteOperations
            .write_file(&target, b"written")
            .await
            .unwrap();
        assert_eq!(
            std::fs::read_to_string(root.path().join("authorized.txt")).unwrap(),
            "written"
        );
        assert_eq!(
    ...
    #[tokio::test]
    async fn authorization_bound_vacant_write_rejects_leaf_substitution() {
        let root = tempfile::tempdir().unwrap();
        let capability = FilesystemCapability::new(root.path().to_path_buf()).unwrap();
        capability
            .bind_tool_target("op", "call", "write", "target.txt")
        ...
        let target = capability
            .take_bound_tool_target("op", "call", "write", "target.txt")
            .unwrap();
        let error = RealWriteOperations
            .write_file(&target, b"written")
            .await
            .unwrap_err();
        assert!(error.contains("failed to create authorization-bound target"));
        assert_eq!(
            std::fs::read_to_string(root.path().join("target.txt")).unwrap(),
            "replacement"
        );
    ...
    #[tokio::test]
    async fn workspace_directory_handle_survives_root_path_replacement() {
        let parent = tempfile::tempdir().unwrap();
        let root = parent.path().join("workspace");
        let moved = parent.path().join("moved-workspace");
        std::fs::create_dir(&root).unwrap();
        std::fs::write(root.join("target.txt"), "authorized").unwrap();
        let capability = FilesystemCapability::new(root.clone()).unwrap();
        ...
        let target = capability
            .prepare_target_for_tool("read", "target.txt")
            .await
            .unwrap();
        let bytes = RealReadOperations.read_file(&target).await.unwrap();
        assert_eq!(bytes, b"authorized");
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn external_symlink_cannot_be_read_or_mutated_through_workspace_capability() {
        ...
        let read = capability
            .prepare_target_for_tool("read", "linked.txt")
            .await
            .unwrap();
        assert!(RealReadOperations.read_file(&read).await.is_err());
        assert!(
            capability
                .prepare_target_for_tool("write", "linked.txt")
                .await
                .is_err()
        );
        assert_eq!(std::fs::read_to_string(secret).unwrap(), "external-secret");
    }
}
