...
pub(crate) struct PendingPromptControlCleanupGuard {
...
pub(crate) struct PromptControlCleanupGuard {
    cleanup: PromptControlCleanup,
...
impl CodingAgentSession {
    pub(crate) fn pending_prompt_control_cleanup_guard(
        &self,
    ) -> Result<Option<PendingPromptControlCleanupGuard>, CodingSessionError> {
        let registration = self
            .runtime_host
            .operation_supervisor
            .control
            .current_prompt_control_registration()?;
        Ok(registration.map(|registration| {
            PendingPromptControlCleanupGuard::new(
                self.runtime_host
                    .operation_supervisor
                    .control
