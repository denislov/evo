...
mod tests {
    ...
    #[test]
    fn builtins_project_stable_semantic_colors() {
        let dark = CodingAgentThemeSnapshot::dark();
        let light = CodingAgentThemeSnapshot::light();

        assert_eq!(dark.name(), "dark");
        assert_eq!(light.name(), "light");
        assert_ne!(
            dark.foreground(CodingAgentThemeForeground::Accent),
            light.foreground(CodingAgentThemeForeground::Accent)
        );
        assert_ne!(
            dark.background(CodingAgentThemeBackground::UserMessage),
            CodingAgentResolvedColor::Default
        );
    }

    #[test]
    fn debug_omits_theme_filesystem_authority() {
        let controller =
            CodingAgentThemeController::from_internal(PathBuf::from("/secret/product/themes"));
        let debug = format!("{controller:?}");

        assert!(!debug.contains("secret"));
        assert!(!debug.contains("themes_dir"));
    }

    #[test]
    fn custom_selection_returns_only_a_resolved_snapshot() {
        let temp = tempfile::tempdir().unwrap();
        std::fs::write(temp.path().join("violet.json"), crate::theme::DARK_JSON).unwrap();
        let controller = CodingAgentThemeController::from_internal(temp.path().to_path_buf());

        let snapshot = controller.select("violet").unwrap();
        assert_eq!(snapshot.name(), "violet");
        assert_eq!(
            snapshot.foreground(CodingAgentThemeForeground::Accent),
            CodingAgentThemeSnapshot::dark().foreground(CodingAgentThemeForeground::Accent)
        );

        std::fs::write(temp.path().join("invalid.json"), "not json").unwrap();
        let error = controller.select("invalid").unwrap_err();
