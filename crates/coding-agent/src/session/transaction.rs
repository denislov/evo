...
impl SessionTransactionWriter {
    ...
    #[cfg(test)]
    pub(crate) fn pause_last_owner_release_for_tests(
        &self,
        ...
        *self
            ...
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner()) = Some((entered, release_receiver));
...
mod tests {
    ...
    fn bounded_writer_rejects_when_queue_is_saturated() {
        ...
        let writer = SessionTransactionWriter::new(store, handle).unwrap();
    ...
    fn independent_session_writers_progress_while_one_writer_is_blocked() {
        ...
        let writer_one = SessionTransactionWriter::new(store_one, handle_one).unwrap();
        let writer_two = SessionTransactionWriter::new(store_two, handle_two).unwrap();
        let (entered_sender, entered_receiver) = sync_channel(1);
        let (release_sender, release_receiver) = sync_channel(1);
        let (reply, _response) = sync_channel(1);
    ...
    #[test]
    fn writer_scans_sequence_once_then_assigns_append_cursors_in_constant_time() {
        let (_temp, store, handle) = setup();
        let writer = SessionTransactionWriter::new(store.clone(), handle.clone()).unwrap();
        assert_eq!(store.sequence_scan_count(), 1);

        for index in 0..64 {
            writer
                .append_checkpoint_events(vec![SessionEventEnvelope::new(
                    "sess_tx",
        ...
        assert_eq!(store.sequence_scan_count(), 1);
        assert_eq!(writer.committed_session_sequence(), 64);
        assert_eq!(
            store
                .read_events(&handle)
                .unwrap()
    ...
    #[test]
    fn process_writer_lease_helper() {
        let Some(root) = std::env::var_os("EVO_TEST_SESSION_WRITER_ROOT") else {
            return;
        };
        let ready = PathBuf::from(
            std::env::var_os("EVO_TEST_SESSION_WRITER_READY")
                .expect("helper ready path is configured"),
        );
        let release = PathBuf::from(
            std::env::var_os("EVO_TEST_SESSION_WRITER_RELEASE")
                .expect("helper release path is configured"),
        );
        let store = SessionLogStore::new(root);
        let handle = store.open_session_id("sess_process_writer").unwrap();
        let writer = SessionTransactionWriter::new(store, handle).unwrap();
        ...
        while !release.exists() {
            assert!(
                ...
                "parent did not release helper writer lease"
            );
    ...
    #[test]
    fn os_writer_lease_rejects_a_second_process_and_releases_on_exit() {
        let temp = tempfile::tempdir().unwrap();
        let store = SessionLogStore::new(temp.path());
        let handle = store
        ...
        let mut child = std::process::Command::new(std::env::current_exe().unwrap())
            .arg("--exact")
            .arg("session::transaction::tests::process_writer_lease_helper")
            .arg("--nocapture")
            .env("EVO_TEST_SESSION_WRITER_ROOT", temp.path())
            .env("EVO_TEST_SESSION_WRITER_READY", &ready)
            .env("EVO_TEST_SESSION_WRITER_RELEASE", &release)
            .spawn()
            .unwrap();
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
        while !ready.exists() {
            if std::time::Instant::now() >= deadline {
                let _ = child.kill();
                panic!("writer helper did not acquire its process-visible lease");
            }
            std::thread::sleep(std::time::Duration::from_millis(10));
        }

        let error =
            SessionTransactionWriter::new(SessionLogStore::new(temp.path()), handle.clone())
                .unwrap_err();
        assert!(
            error.to_string().contains("writer in another process"),
        ...
        let writer =
            SessionTransactionWriter::new(SessionLogStore::new(temp.path()), handle).unwrap();
        writer.shutdown().unwrap();
    }
                    ...
                    crate::events::CodingAgentDiagnosticProductEvent::Diagnostic {
                        diagnostic:
                            crate::runtime::public_error::CodingAgentPublicDiagnostic::new(
                                crate::runtime::public_error::CodingAgentPublicDiagnosticSeverity::Info,
                                "outbox_test",
                                "outbox test",
                                crate::runtime::public_error::CodingAgentPublicDiagnosticOrigin::Persistence,
                                Some(operation_id),
                            ),
                    },
                ),
    ...
    fn event_kinds(events: &[SessionEventEnvelope]) -> Vec<&'static str> {
        events
            ...
            .map(|event| match event.data {
                ...
                SessionEventData::OperationRecoveryPending { .. } => "operation.recovery_pending",
                SessionEventData::OperationRecoveryResolved { .. } => "operation.recovery_resolved",
