...
pub(crate) enum CodingSessionError {
    ...
    #[error("session write failed ({reason:?}): {message}")]
    SessionWriteFailure {
        reason: SessionWriteFailureReason,
        message: String,
    },
    #[error(
...
impl CodingSessionError {
    pub fn code(&self) -> &'static str {
        match self {
            ...
            Self::SessionWriteFailure { .. } => "session_write_failure",
