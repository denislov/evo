...
impl SubmissionCommitGuard {
    pub(super) fn commit_execution(
        ...
        *self.lifecycle.lock_resource("submission lease lifecycle")? =
...
impl Drop for SubmissionCommitGuard {
    fn drop(&mut self) {
        ...
        if let Some(execution) = self.execution.as_ref() {
            ...
            // Drop cannot surface a resource error; recover only to abandon
            // the lease and report poison once.
            let mut lifecycle = self.lifecycle.lock_or_recover("submission lease lifecycle");
...
impl CodingAgentSession {
    ...
    pub(crate) async fn run_internal(
        ...
        let _pending_prompt_control_cleanup = if matches!(
            &operation,
            CodingAgentOperation::Prompt(_) | CodingAgentOperation::InvokeAgent(_)
        ) {
            self.pending_prompt_control_cleanup_guard()?
        } else {
            None
        };
        self.runtime_host
    ...
    pub(crate) fn install_submission_lease(
        ...
        if let Some(pending) = &self.runtime_host.client_projection.pending_submission {
            let lifecycle = *pending
                .lifecycle
                .lock_resource("submission lease lifecycle")?;
    ...
    pub(super) fn consume_submission_lease(
    ...
    ) -> Result<Option<SubmissionCommitGuard>, CodingSessionError> {
        ...
        *pending
            .lifecycle
            .lock_resource("submission lease lifecycle")? = SubmissionLeaseLifecycle::Consuming;
        Ok(Some(SubmissionCommitGuard {
