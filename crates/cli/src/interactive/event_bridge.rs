...
mod cross_adapter_tests {
    ...
    use coding_agent::api::view::{
        CapabilityStatus, CodingAgentCapabilities, CodingAgentSessionView, ProfileId,
    };
    ...
    fn snapshot() -> CodingAgentSnapshot {
        CodingAgentSnapshot {
    ...
    #[test]
    fn shared_cross_adapter_fixture_matches_interactive_product_state_exactly() {
        let snapshot = snapshot();
        let mut shared =
            CodingAgentClientProjection::new(snapshot.clone()).expect("valid shared snapshot");
        let mut interactive = UiProjection::from_snapshot(snapshot);

        for event in cross_adapter_fixture_events() {
            assert!(matches!(
                shared.apply(&event),
                CodingAgentClientProjectionApply::Applied(_)
            ));
        ...
        assert_eq!(
            interactive
                .product
                .as_ref()
                .expect("snapshot-backed interactive projection"),
            &shared
        );
        assert_eq!(interactive.context(), &shared.snapshot().context);
        assert_eq!(shared.snapshot().cursor.last_event_sequence, 13);
        assert_eq!(
            shared.snapshot().context.operations[0].status,
            coding_agent::api::client::CodingAgentOperationStatus::Completed
        );
        assert_eq!(shared.messages()[0].text, "hello world");
        assert_eq!(shared.tools()[0].detail, "read complete");
        assert_eq!(shared.snapshot().context.delegations[0].status, "completed");
        assert_eq!(
            shared.snapshot().session.default_agent_profile_id.as_str(),
            "reviewer"
        );
    }
}
