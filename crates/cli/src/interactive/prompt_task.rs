...
mod ownership_tests {
    ...
    use super::{
        PROMPT_TASK_CONTROL_CAPACITY, PromptTask, PromptTaskControl, PromptTaskControlHandle,
    };

    fn task_function_body<'a>(source: &'a str, function_name: &str) -> &'a str {
    ...
    #[test]
    fn interactive_tasks_handoff_one_connection_without_local_subscription() {
        ...
        for function_name in [
            ...
            let body = task_function_body(source, function_name);
            assert!(!body.contains(&product_subscription));
            assert!(body.contains("prepare_interactive_submission("));
            assert!(
