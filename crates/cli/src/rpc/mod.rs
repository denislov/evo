...
mod public_error_tests {
    ...
    #[test]
    fn rpc_session_error_drops_raw_provider_body_credentials_and_path() {
        const SECRET: &str = "rpc-provider-secret-canary";
        let response = rpc_public_error(
            Some("request-1".into()),
            "prompt",
            CodingAgentPublicError {
        ...
        assert_eq!(
            ...
            Some("The model provider request failed.")
        );
        assert_eq!(response.data.as_ref().unwrap()["category"], "provider");
        assert_eq!(response.data.as_ref().unwrap()["code"], "provider");
        assert_eq!(response.data.as_ref().unwrap()["retryable"], true);
    ...
    #[test]
    fn rpc_cli_error_drops_raw_session_body_credentials_and_path() {
        const SECRET: &str = "rpc-cli-secret-canary";
        let response = rpc_cli_error(
            Some("request-2".into()),
            "new_session",
        ...
        assert_eq!(
            response.error.as_deref(),
            Some("The session request failed.")
        );
        assert_eq!(response.data.as_ref().unwrap()["category"], "session");
        assert_eq!(response.data.as_ref().unwrap()["code"], "session_failure");
        assert!(!serialized.contains(SECRET));
        assert!(!serialized.contains("/private"));
    }
}
