...
mod tests {
    ...
    fn done_event(content: Vec<ContentBlock>) -> AssistantMessageEvent {
    ...
    #[test]
    fn provider_event_budget_enforces_event_and_content_totals() {
        let limits = ProviderEventLimits {
            events: 1,
            content_blocks: 1,
            content_bytes: 4,
            tool_calls: 1,
            tool_argument_bytes: 8,
        };
        let text = done_event(vec![ContentBlock::Text {
            text: "four".into(),
            text_signature: None,
        }]);
        let mut budget = ProviderEventBudget::new(limits);

        assert_eq!(budget.observe(&text), Ok(()));
        assert_eq!(budget.observe(&text), Err(ProviderLimit::Events));

        let oversized = done_event(vec![ContentBlock::Text {
            text: "five!".into(),
            text_signature: None,
        }]);
        assert_eq!(
            ProviderEventBudget::new(limits).observe(&oversized),
            Err(ProviderLimit::ContentBytes)
        );
    }

    #[test]
    fn provider_event_budget_counts_tool_calls_and_serialized_arguments_without_cloning() {
        let tool = done_event(vec![ContentBlock::ToolCall {
            id: "call".into(),
            name: "read".into(),
        ...
        assert_eq!(
            ProviderEventBudget::new(ProviderEventLimits {
                events: 1,
                content_blocks: 1,
                content_bytes: 16,
                tool_calls: 0,
                tool_argument_bytes: 1024,
            })
            .observe(&tool),
            Err(ProviderLimit::ToolCalls)
        );
        assert_eq!(
            ProviderEventBudget::new(ProviderEventLimits {
                events: 1,
                ...
                content_bytes: 16,
                tool_calls: 1,
                tool_argument_bytes: 4,
            })
            .observe(&tool),
            Err(ProviderLimit::ToolArgumentBytes)
        );
    }

    #[tokio::test]
    async fn provider_event_limit_terminates_the_stream_with_one_typed_failure() {
        let inner: EventStream = Box::pin(futures::stream::iter(vec![done_event(vec![
            ContentBlock::Text {
                text: "too large".into(),
                text_signature: None,
            },
        ])]));
        let events = enforce_provider_event_limits(
            inner,
            "test-api".into(),
            budget_model(),
            ProviderEventLimits {
        ...
        let AssistantMessageEvent::Error { reason, message } = &events[0] else {
            panic!("expected one limit error, got {events:?}");
        };
        assert_eq!(*reason, StopReason::Error);
        assert_eq!(
            message.error_message.as_deref(),
            Some("Provider response exceeded the content bytes limit")
        );
    }
}
