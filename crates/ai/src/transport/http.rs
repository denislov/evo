...
mod tests {
    ...
    async fn invocation_deadline_bounds_retry_backoff_under_fake_time() {
        let deadline = InvocationDeadline {
        ...
        assert_eq!(result, Err(WaitError::Timeout { timeout_ms: 100 }));
    }

    #[tokio::test(start_paused = true)]
    async fn cancellation_wins_retry_backoff_without_advancing_time() {
