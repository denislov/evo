...
- [ ] 将 `Rc<Vec<Size<Pixels>>>` 从 render 局部变量移入持久 list model。
- [ ] append 只增加一项，streaming 只更新一个 index。
- [ ] 宽度 bucket 改变时批量 invalidate；resize 使用 50–80ms trailing debounce。
- [ ] 不在每帧为 10,000 条消息重新构造、比较和展开尺寸数组。
