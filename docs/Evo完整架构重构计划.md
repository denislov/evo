...
| Phase 5 / ARC-500 | 完成（复验 2026-08-06） | `docs/refactor/phase5-agent-actor.md`；`Arc<RwLock<AgentState>>`、`queues_cleared`、`RunGuard`、`TurnRunDropGuard` 已删除；Agent 持有 `AgentHandle`（bounded mpsc），actor 独占 AgentState；`TurnRunner` 替代 `TurnLoopStream`，`turn_continues` flag 在 tool-turn 间让出控制权；coding-agent async 适配；复验修复：turn 进行中收到 Shutdown 不再跳过 commit（drain loop 改为 flag+break 走 graceful shutdown）、event send 阻塞期间继续处理 mailbox（消除 consumer 等 reply 时的循环等待）；`runtime.rs` actor 实现拆至 `actor.rs`；agent-core 77、coding-agent 185 测试通过 |
| Phase 5 / ARC-510 | 完成（复验 2026-08-06） | `docs/refactor/phase5-prompt-queue.md`；`PromptQueueEntry`（id+version+message）替换裸 `AgentMessage`；`AgentInputQueue::Interjection` 高优先级 drain；`AgentQueueError::StaleVersion`/`NotFound` 支持 edit/remove 带版本检查；coding-agent `CodingAgentControlKind::Interject` 适配；复验修复：steer/follow-up/interject 由 fire-and-forget 改为 async request（入队 ItemLimit/ByteLimit 失败不再静默）、turn 进行中 pending 缓冲同样受容量限制、`flush_pending`/commit 不再丢弃未 flush 的 pending 输入 |
...
| Phase 7 Gate | 通过（复验 2026-08-07；GPT 审查修复 2026-08-07） | 用户扩展不能绕过 trust/authorization/sandbox：hook 与 MCP stdio server 进程强制 SandboxProfile 且能力不足平台 fail-closed（仅 sandbox 不支持对 Tool gate fail-closed，其余 fail-open 有 transition 表）；extension 修改文件经 `HookEdit` 归因进入 hunk review 且可 accept/reject；首次启用必须展示来源和能力（EnableRequest + 诊断事件）。extension-host 220、coding-agent 255、agent-core 77、workspace-runtime 127、change-tracker 66、cli 106、tui 34、ai 113、event-journal 4、tool-contract 12、tool-runtime 9 测试通过（gate.sh 复验除既有问题外无失败；coding-agent 并行验证用 `--test-threads=4` 规避既有 flock flaky）；全 workspace clippy/fmt/architecture gate 通过（oversized_debts=36 既有基线、execution_debts=0）；MCP 工具与内建工具共用 Tool contract/事件/取消语义（`mcp_search`/`mcp_use` 经 typed ToolRegistry 注册，cancel 贯通）；既有全仓 Gate 问题延续：desktop `desktop-runtime` 测试线程 stack overflow（Phase 4 登记）、coding-agent session writer flock 并行 flaky（ARC-710 登记）；**2026-08-07 GPT 审查修复（7 项，commits `71eec91`/`dc45412`）**：① hook 修改文件自动归因 `HookEdit`（`HookLifecycle` before/after 注入 seam + coding-agent `hook_attribution` observer：before 基线/after 对比 → `record_receipt(HookEdit)`，因果窗口先到先得、窗口过期保持 `ExternalEdit`；e2e 测试改为真自动归因）；② OAuth refresh 后新 access token 注入 HTTP 请求（`request_with_headers`/`notify_with_headers`，动态覆盖静态同名 header，stdio 不注入）；③ MCP 并发上限强制（`McpHost.set_max_concurrent_extensions`，默认 32，超限截断 + `mcp_concurrency_limit` 诊断）；④ 带参数相对 hook command 相对 `source_dir` 解析（shell 分支第一 token 相对路径绝对化，PATH 命令/`$`/`~`/管道/重定向不替换）；⑤ Tool/Stop gate 输出超预算不再用截断尾部驱动决策（统一 `OutputLimited` fail-open）；⑥ MCP transport 输入边界（单行 1 MiB/累计 64 MiB/HTTP body 64 MiB，超限 fail-closed 终止 transport）；⑦ manifest 内嵌 `config` per-extension 生效（外部层 > manifest > host-defaults，`enabled:false` 禁用 + `extension_disabled` 诊断，per-extension budget 覆盖 hook timeout；MCP 不做 per-extension budget，理由见 `phase7-extension-host.md`）；验证债务登记：HTTP hook runner（最迟 Phase 10）、MCP HTTP SSE/streaming、MCP resources 深度支持、ACP transport（仅真实需求才加入）、MCP 跨进程 credential flock、产品侧 OAuth device flow 引导 UI（Phase 9）、MCP 请求级取消（协议未定义）、stderr 日志呈现（Phase 9）、首次启用确认路径（Phase 9 CLI） |
...
- `interactive/root.rs` 已按 state/bootstrap、menus/authorization、session/render state、settings、shell input、shell rendering、context、transcript 与 layout 拆为 9 个子模块；父文件降至 631 行，所有 production 子模块均低于 900 行，并清偿 oversized debt。
