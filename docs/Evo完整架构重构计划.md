...
| Phase 4 / ARC-400 | 完成（复验 2026-08-06） | `docs/refactor/phase4-change-tracker.md`；`change-tracker` crate（仅依赖 workspace-runtime）：单 actor fs event service、bounded debounce、directory type fact、dynamic directory 补扫、rename chain/ambiguous/root/ignore 边界、watch budget fail-closed、可靠 shutdown、带 root/sequence 的 Git HEAD/index/lock 元数据事件，31 个 watcher 测试覆盖 |
