...
This document records the repeatable headless and opt-in native release gates introduced for DESK-017. The native gate measures GPUI's real `draw + platform_window.draw/present` boundary; neither gate claims click-to-photon input latency.
...
It builds the real release binary, opens a deterministic 1,300×900 NativeShell containing 10,000 transcript blocks, warms 20 frames, forces 200 further redraws, and exits automatically. GPUI's `ZED_MEASUREMENTS` hook brackets `Window::draw` and `Window::present`, including the platform renderer's draw/present call. The script normalizes GPUI's duration units, requires exactly 200 post-warmup samples, asserts P95 against 16.7 ms, and writes `target/desktop-perf/native-latest.log`. It intentionally fails early when neither `DISPLAY` nor `WAYLAND_DISPLAY` is available instead of silently substituting the headless platform.
...
| Same 10,000-block fixture in a real X11 window, 200 post-warmup forced redraws | native GPU/present frame P95 | 2,902 µs |
| Same native replay | platform frame callback cadence P95 | 8,389 µs |
...
The separate native replay uses the production binary rather than a GPUI test binary because only the production platform loop exposes GPUI's internal `frame duration` measurement. It records an exact P95 for the locked GPUI draw/present boundary and an informational callback-cadence P95; cadence reflects display refresh scheduling and is not added to draw/present time. DESK-017 remains open for click-to-photon input latency, native P99 policy, and a cross-platform resident-memory curve. DESK-018 will own screenshot/golden coverage, including wide/medium/narrow layouts and authorization/recovery/file-review flows.
