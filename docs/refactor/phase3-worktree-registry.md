...
- `create_managed` 高层入口：由 source + owner + 纳秒时间、进程 id 和序列生成唯一 id（`child-<hex>`），先持久化 `Creating`，再调 `WorktreeBuilder`（显式 `worktree_id` 钉住 handle id == 目录名 == 记录 id），materialize 成功后原子写入 `Ready`；失败窗口保留可恢复的 `Creating` record。
...
60 passed, 0 failed（新增 20 个 registry/安全边界测试）
