...
| OAuth 401 refresh | **本次新增** | `mcp_lifecycle.rs::oauth_401_refreshes_token_and_retries_once`（成功）、`oauth_401_refresh_failure_falls_back_to_device_flow_and_surfaces_error`（失败） | 401 → refresh（mock token 端点）→ retry 成功且凭据轮换落库；refresh 失败 → device flow 兜底失败 → 结构化 `Unauthorized` + `mcp_refresh_failed` 诊断（并修复了上述 transport 缺陷） |
...
| extension 修改文件归因 + hunk review | **本次新增** | `coding-agent/.../hooks_e2e_tests.rs::hook_edit_is_attributed_and_reviewable_end_to_end` | 真实 hook 进程写工作区文件 → review tracker `record_receipt`（`ChangeSource::HookEdit`）→ `list_changes` 可见（source=hook_edit）→ `open_change` 可读 → `accept_hunk` 生效（accepted 变更离开 review 列表）→ 第二次 extension 修改 → `reject_hunk` 生效（文件回退 accepted baseline） |
